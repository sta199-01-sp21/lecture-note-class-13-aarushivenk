...
The natural point estimate would be mean, $\bar{x}$ . 
...
The downside is that it is harder to get the right answer, whereas an interval 
provides more of a range, that increases accuracy but decreases precision. 
...
We would expect 5 students to be left handed after the initial sample. I
wouldn't be surprised if only 3 were left handed because there is not that much 
variability (10% of sample to 6%). If 40 of them were left handed, that would be
very surprising and suggest that we had a biased sample. 
...
```{r data_manhattan}
manhattan <- read_csv("~/R/lecture-note-class-13-aarushivenk/manhattan.csv")
```
...
A 95% confidence interval means that in repeated samples, we are 95% confident 
that our interval contains the true mean. 
...
**Interpretation**: We are 95% confident that the mean is in between 2299 and 
2977. 
...
```
We are 95% confident that the median is between 2162 and 2875. 
...
**Interpretation**: We are 95% confident that the true proportion of mice that
died while not on treatment is between 69% and 79.7%. 
...
```{r practice_2_ci}
ci_mice_lived <- get_ci(boot_prop_treatment, level = .90)
ci_mice_lived
...
We are 90% confident that the true proportion of mice that lived while on
